...
export default function AdminContactsPage() {
  ...
  const { data, isLoading } = useQuery<
    ApiResponse<{ contacts: AdminContact[]; pagination: Pagination }>
  >({
        ...
        <>
          <div className="space-y-3 mb-6">
            {contacts.map((contact) => (
              <div
                key={contact._id}
                className="bg-white dark:bg-neutral-800 rounded-xl sm:rounded-2xl shadow-sm border border-neutral-100 dark:border-neutral-700 overflow-hidden hover:shadow-md transition-all duration-300"
              >
                <div
                  className="flex items-start justify-between p-4 sm:p-5 cursor-pointer gap-2"
                  onClick={() =>
                    setExpanded(expanded === contact._id ? null : contact._id)
                  }
                >
                  <div className="flex items-start gap-2.5 sm:gap-3 min-w-0">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-cyan-100 dark:bg-cyan-500/20 flex items-center justify-center shrink-0 mt-0.5">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs sm:text-sm font-medium text-neutral-700 dark:text-neutral-300 mt-1 sm:mt-1.5">
                        {contact.subject}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-[10px] sm:text-xs text-neutral-400 hidden sm:block">
                ...
                {expanded === contact._id && (
                  <div className="px-4 sm:px-5 pb-4 sm:pb-5 pt-0">
                    <div className="sm:pl-[52px]">
                      <p className="text-xs sm:text-sm text-neutral-600 dark:text-neutral-300 leading-relaxed whitespace-pre-wrap bg-neutral-50 dark:bg-neutral-700/50 rounded-xl p-3 sm:p-4">
                        {contact.message}
                      </p>
                      <p className="text-[10px] sm:text-xs text-neutral-400 mt-2 sm:hidden">
