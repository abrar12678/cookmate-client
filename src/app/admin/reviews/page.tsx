...
export default function AdminReviewsPage() {
  ...
  const { data, isLoading } = useQuery<
    ApiResponse<{ reviews: AdminReview[]; pagination: Pagination }>
  >({
  ...
  const renderStars = (rating: number) => (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
          key={star}
          className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${star <= rating ? "fill-yellow-400 text-yellow-400" : "text-neutral-300 dark:text-neutral-600"}`}
        />
      ))}
    </div>
  );
        ...
        <>
          <div className="space-y-3 mb-6">
            {reviews.map((review) => (
              <div
                key={review._id}
                className="bg-white dark:bg-neutral-800 rounded-xl sm:rounded-2xl p-4 sm:p-5 shadow-sm border border-neutral-100 dark:border-neutral-700 hover:shadow-md transition-all duration-300"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2.5 sm:gap-3 mb-2">
                      <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-primary-100 dark:bg-primary-500/20 flex items-center justify-center text-[10px] sm:text-xs font-bold text-primary-600 dark:text-primary-400 shrink-0">
                        {review.userName?.charAt(0).toUpperCase()}
                      </div>
                      <div className="min-w-0">
                        <p className="text-xs sm:text-sm font-semibold text-neutral-800 dark:text-neutral-100 truncate">
                          {review.userName}
                        </p>
                        <p className="text-[11px] sm:text-xs text-neutral-400 truncate">
                          on{" "}
                          <Link
                            href={`/recipe/${review.recipeId}`}
                    ...
                    <p className="text-[10px] sm:text-xs text-neutral-400 mt-2">
